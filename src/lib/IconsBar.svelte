<script lang="ts">
  ...
  $: useTwitchSizeChat = $combinedChatsConfigStore.useTwitchChatSize;

  function setUseTwitchChatSize() {
    combinedChatsConfigStore.setUseTwitchChatSize(useTwitchSizeChat);
  }
...
<div class="icons-bar d-flex flex-row mx-3">
  {#if $page.url.pathname.includes("combined-chats-readonly")}
    <div class="d-flex flex-row w-auto align-items-center">
      <div class="switch-container">
        <input
          ...
          class="checkbox"
          id="checkbox"
          bind:checked={useTwitchSizeChat}
          on:change={setUseTwitchChatSize}
        />
        <label class="switch" for="checkbox">
          <span class="slider"></span>
        </label>
      </div>
      <span class="ms-2">Twitch Size Chat</span>
    </div>
  {/if}
  <div class="ms-auto">
    <button
